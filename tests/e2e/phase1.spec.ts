...
test("cross-season search stays local and matches anime, songs, and artists", async ({ page }) => {
  ...
  await expect(page.locator("[data-catalog-anime-count]")).toHaveText("1956");
...
test("catalogue navigation stays compact and drills down through published years and quarters", async ({ page }) => {
  ...
  await expect(page.getByLabel("季度資料狀態")).toContainText("已發布 29 個季度、1,956 個作品頁與 4,348 首 OP／ED");
...
test("2019 spring browsing and creator search reach the correct special ending on desktop and mobile", async ({ page }) => {
  for (const width of [1280, 390]) {
    await page.setViewportSize({ width, height: 844 });
    await page.goto("/catalog/2019/");
    await page.getByRole("link", { name: /春季動畫.*39 套動畫/ }).click();
    await expect(page.getByRole("heading", { name: "2019 春季動畫" })).toBeVisible();
    await expect(page.getByText("本季正在補充，目前收錄 38 套 TV 作品及 1 套網絡連載（含電視短篇及重編版）。其餘作品、跨季延續及特殊歌曲版本仍待核對。", { exact: true })).toBeVisible();
    await expect(page.locator("[data-anime-card]")).toHaveCount(39);
    await expect(page.locator("[data-anime-card] img")).toHaveCount(0);
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
...
test("PriChan second-season search preserves opening versions, character endings and shared creators across devices", async ({ page, browser, request }) => {
  test.setTimeout(60_000);
  ...
  const detail = await response.json();
  expect(detail.id).toBe("catalog-kiratto-prichan-2-2019");
  expect(detail.themes.map(({ releaseDate }: { releaseDate: string }) => releaseDate)).toEqual(["2019-05-29", "2019-11-27", "2020-06-24", "2020-06-24"]);
  for (const width of [1280, 960, 390, 320]) {
    await page.setViewportSize({ width, height: 844 });
    await page.goto("/search/", { waitUntil: "domcontentloaded" });
    await page.getByRole("searchbox", { name: "搜尋動畫、歌曲或創作者" }).fill("閃躍吧");
    await page.getByLabel("年份", { exact: true }).selectOption("2019");
    await page.getByLabel("季度", { exact: true }).selectOption("spring");
    await expect(page.locator("[data-catalog-result]")).toHaveCount(1);
    await page.getByRole("link", { name: "キラッとプリ☆チャン シーズン2", exact: true }).first().click();
    await expect(page.locator(".anime-hero__zh")).toHaveText("閃躍吧！星夢頻道 第二季");
    await expect(page.locator(".anime-hero__meta")).toContainText("每週日 10:00");
    await expect(page.locator(".theme-card")).toHaveCount(4);
    const opening = page.locator(`#theme-${slug}-op-2`);
    await expect(opening.locator(".theme-card__artist")).toHaveText("Run Girls, Run！");
    await expect(opening).toContainText("2019 年 10 月起");
    await expect(opening.locator("time")).toHaveAttribute("datetime", "2019-11-27");
    await page.getByRole("link", { name: "搜尋「前田佑」的歌曲", exact: true }).first().focus();
    await page.keyboard.press("Enter");
    await expect(page.getByRole("searchbox", { name: "搜尋動畫、歌曲或創作者" })).toHaveValue("前田佑");
    for (const [label, value] of [["年份", "2019"], ["季度", "spring"], ["歌曲用途", "ED"]]) await page.getByLabel(label!, { exact: true }).selectOption(value!);
    await page.getByRole("link", { name: "Brand New Girls", exact: true }).click();
    const ending = page.locator(`#theme-${slug}-ed-2`);
    await expect(ending).toBeFocused();
    for (const name of ["林鼓子", "厚木那奈美", "森嶋優花"]) await expect(ending.getByRole("link", { name: `搜尋「${name}」的歌曲`, exact: true })).toHaveCount(1);
    for (const name of ["栗原暁", "前田佑"]) await expect(ending.getByRole("link", { name: `搜尋「${name}」的歌曲`, exact: true })).toHaveCount(3);
    await expect(ending.locator("time")).toHaveAttribute("datetime", "2020-06-24");
    await expect(ending).toContainText("2020 年歌曲合集 CD");
    await ending.locator("details summary").focus();
    await page.keyboard.press("Enter");
    await expect(ending.locator('a[href="https://utaten.com/lyric/nm20062644/"]')).toBeVisible();
    await expect(page.locator("iframe, .anime-hero img, .anime-hero__romaji")).toHaveCount(0);
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  }
  expect(mediaRequests).toEqual([]);
  const native = await browser.newContext({ javaScriptEnabled: false, viewport: { width: 390, height: 844 } });
  try {
    ...
    await nativePage.getByRole("link", { name: /春季動畫.*39 套動畫/ }).click();
    await nativePage.getByRole("link", { name: "查看 キラッとプリ☆チャン シーズン2（閃躍吧！星夢頻道 第二季）", exact: true }).click();
    await expect(nativePage.locator(".theme-card")).toHaveCount(4);
    await expect(nativePage.locator(`#theme-${slug}-ed-2 .theme-card__artist`)).toContainText("紫藤める（CV.森嶋優花）");
...
test("ULTRAMAN original-season browsing preserves its web identity and missing-song state across devices", async ({ page, browser, request }) => {
  test.setTimeout(60_000);
  const slug = "ultraman-2019";
  const mediaRequests: string[] = [];
  page.on("request", (item) => {
    if (/youtube|ytimg|googlevideo/.test(new URL(item.url()).hostname)) mediaRequests.push(item.url());
  });
  ...
  expect(response.status()).toBe(200);
  const detail = await response.json();
  expect(detail).toMatchObject({ id: "catalog-ultraman-2019", themeAvailability: "not_announced", themes: [] });
  for (const width of [1280, 960, 390, 320]) {
    await page.setViewportSize({ width, height: 844 });
    await page.goto("/search/", { waitUntil: "domcontentloaded" });
    await page.getByRole("searchbox", { name: "搜尋動畫、歌曲或創作者" }).fill("ULTRAMAN");
    await page.getByLabel("年份", { exact: true }).selectOption("2019");
    await page.getByLabel("季度", { exact: true }).selectOption("spring");
    await expect(page.locator("[data-catalog-result]")).toHaveCount(1);
    for (const type of ["OP", "ED"]) {
      await page.getByLabel("歌曲用途").selectOption(type);
      await expect(page.locator("[data-catalog-result]")).toHaveCount(0);
    }
    await page.getByLabel("歌曲用途").selectOption("all");
    await page.getByRole("link", { name: "ULTRAMAN", exact: true }).first().click();
    await expect(page).toHaveURL(new RegExp(`/anime/${slug}/$`));
    await expect(page.locator(".anime-hero__meta")).toContainText("網絡配信第 1 季，全 13 話");
    await expect(page.getByRole("heading", { name: "主題曲資料尚待補充" })).toBeVisible();
    await expect(page.getByRole("heading", { name: "本作不設獨立 OP／ED" })).toHaveCount(0);
    await expect(page.locator(".theme-card, iframe, .anime-hero img, .anime-hero__romaji")).toHaveCount(0);
    const netflixSource = page.getByRole("link", { name: /^Netflix 繁中公告：作品名稱與 2019 年 4 月上線日期 / });
    await expect(netflixSource).toBeVisible();
    await expect(netflixSource).toHaveAttribute("href", "https://about.netflix.com/zh_tw/news/anime-production-line-deal");
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  }
  const native = await browser.newContext({ javaScriptEnabled: false, viewport: { width: 390, height: 844 } });
  try {
    ...
    await nativePage.goto(`${e2eOrigin}/catalog/2019/`);
    await nativePage.getByRole("link", { name: /春季動畫.*39 套動畫/ }).click();
    await nativePage.getByRole("link", { name: "查看 ULTRAMAN", exact: true }).click();
    await expect(nativePage).toHaveURL(new RegExp(`/anime/${slug}/$`));
    await expect(nativePage.getByRole("heading", { name: "主題曲資料尚待補充" })).toBeVisible();
    await expect(nativePage.locator(".theme-card, iframe")).toHaveCount(0);
    expect(await nativePage.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  } finally {
    await native.close();
  }
...
test("Aikatsu Friends second-season search preserves the four-character opening, joint writers and withheld release dates across devices", async ({ page, browser, request }) => {
  test.setTimeout(60_000);
  ...
  expect(response.status()).toBe(200);
  const detail = await response.json();
  expect(detail.id).toBe("catalog-aikatsu-friends-2-2019");
  expect(detail.themes).toHaveLength(2);
  for (const theme of detail.themes) expect(theme).not.toHaveProperty("releaseDate");
  for (const width of [1280, 960, 390, 320]) {
    await page.setViewportSize({ width, height: 844 });
    await page.goto("/search/", { waitUntil: "domcontentloaded" });
    await page.getByRole("searchbox", { name: "搜尋動畫、歌曲或創作者" }).fill("偶像學園Friends");
    await page.getByLabel("年份", { exact: true }).selectOption("2019");
    await page.getByLabel("季度", { exact: true }).selectOption("spring");
    await expect(page.locator("[data-catalog-result]")).toHaveCount(1);
    await page.getByRole("link", { name: "アイカツフレンズ！～かがやきのジュエル～", exact: true }).first().click();
    await expect(page.locator(".anime-hero__zh")).toHaveText("偶像學園Friends！ 第二季");
    await expect(page.locator(".anime-hero__meta")).toContainText("每週四 18:25");
    await expect(page.locator(".theme-card")).toHaveCount(2);
    await expect(page.locator(".theme-card time")).toHaveCount(0);
    const opening = page.locator(`#theme-${slug}-op-1`);
    await expect(opening.locator(".theme-card__artist")).toHaveText("あいね・みお・舞花・エマ from BEST FRIENDS！");
    await expect(opening).toContainText("CD 日期與聲優歌唱署名待核對");
    await opening.locator("details summary").focus();
    await page.keyboard.press("Enter");
    await expect(opening.locator("details")).toHaveAttribute("open");
    await expect(opening.locator('a[href="https://utaten.com/lyric/mi19041917/"]')).toBeVisible();
    await page.getByRole("link", { name: "搜尋「Maozon」的歌曲", exact: true }).first().focus();
    await page.keyboard.press("Enter");
    await expect(page.getByRole("searchbox", { name: "搜尋動畫、歌曲或創作者" })).toHaveValue("Maozon");
    for (const [label, value] of [["年份", "2019"], ["季度", "spring"], ["歌曲用途", "ED"]]) await page.getByLabel(label!, { exact: true }).selectOption(value!);
    await page.getByRole("link", { name: "Be star", exact: true }).click();
    const ending = page.locator(`#theme-${slug}-ed-1`);
    await expect(ending).toBeFocused();
    await expect(ending.locator(".theme-card__artist")).toHaveText("ひびき from BEST FRIENDS！");
    for (const name of ["Maozon", "YUKI FUNAKOSHI"]) await expect(ending.getByRole("link", { name: `搜尋「${name}」的歌曲`, exact: true })).toHaveCount(2);
  ...
  try {
    ...
    await nativePage.getByRole("link", { name: /春季動畫.*39 套動畫/ }).click();
    await nativePage.getByRole("link", { name: "查看 アイカツフレンズ！～かがやきのジュエル～（偶像學園Friends！ 第二季）", exact: true }).click();
    await expect(nativePage).toHaveURL(new RegExp(`/anime/${slug}/$`));
    await expect(nativePage.locator(".theme-card")).toHaveCount(2);
    await expect(nativePage.locator(".theme-card time")).toHaveCount(0);
    await nativePage.locator(`#theme-${slug}-ed-1 details summary`).focus();
    await nativePage.keyboard.press("Enter");
    await expect(nativePage.locator(`#theme-${slug}-ed-1 a[href="https://utaten.com/lyric/mi19082003/"]`)).toBeVisible();
    await expect(nativePage.locator("iframe")).toHaveCount(0);
    expect(await nativePage.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  } finally {
    await context.close();
...
test("Hangyaku second season preserves original song versions, missing credits and responsive source navigation", async ({ page, browser, request }) => {
  ...
  expect(response.status()).toBe(200);
  const detail = await response.json();
  expect(detail.id).toBe("catalog-hangyakusei-million-arthur-2-2019");
  expect(detail.themes.map(({ titleJa }: { titleJa: string }) => titleJa)).toEqual(["OPEN THE WORLDS", "PEARLY×PARTY"]);
  expect(detail.themes[1].credits).toEqual([]);
  for (const width of [1280, 960, 390, 320]) {
    await page.setViewportSize({ width, height: 844 });
    await page.goto("/search/", { waitUntil: "domcontentloaded" });
    await page.getByRole("searchbox", { name: "搜尋動畫、歌曲或創作者" }).fill("叛逆性百萬亞瑟王");
    await page.getByLabel("年份", { exact: true }).selectOption("2019");
    await page.getByLabel("季度", { exact: true }).selectOption("spring");
    await expect(page.locator("[data-catalog-result]")).toHaveCount(1);
    await page.getByRole("link", { name: /叛逆性ミリオンアーサー 第2シーズン/ }).first().click();
    await expect(page).toHaveURL(new RegExp(`/anime/${slug}/$`));
    await expect(page.locator(".anime-hero__zh")).toHaveText("叛逆性百萬亞瑟王 第二季");
    await expect(page.locator(".anime-hero__romaji")).toHaveCount(0);
    await expect(page.locator(".theme-card")).toHaveCount(2);
    await expect(page.locator(".theme-card time")).toHaveText(["2019-04-24", "2019-05-22"]);
    const ending = page.locator(`#theme-${slug}-ed-1`);
    await expect(ending).toContainText("個別演唱及製作署名待核對");
    await expect(ending.locator(".theme-card__credits dd")).toHaveText("待確認");
    await expect(ending.getByRole("button", { name: /載入 YouTube 影片/ })).toHaveCount(0);
    await expect(ending.locator("details")).toHaveCount(0);
    await expect(ending.locator('a[href="https://catalog.bandainamcomusiclive.co.jp/release/68378/"]')).toBeVisible();
    await page.getByRole("link", { name: "搜尋「小島英也」的歌曲", exact: true }).first().focus();
    await page.keyboard.press("Enter");
    await expect(page.getByRole("searchbox", { name: "搜尋動畫、歌曲或創作者" })).toHaveValue("小島英也");
    for (const [label, value] of [["年份", "2019"], ["季度", "spring"], ["歌曲用途", "OP"]]) await page.getByLabel(label!, { exact: true }).selectOption(value!);
    await expect(page.locator("[data-catalog-result]")).toHaveCount(1);
    await page.getByRole("link", { name: "OPEN THE WORLDS", exact: true }).click();
    const opening = page.locator(`#theme-${slug}-op-1`);
    await expect(opening).toBeFocused();
    await expect(opening).toContainText("mora 先行配信：2019-04-11");
    await expect(page.locator("iframe")).toHaveCount(0);
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  }
  expect(mediaRequests).toEqual([]);
  await page.route("https://www.youtube-nocookie.com/**", (route) => route.fulfill({ contentType: "text/html", body: "<!doctype html><title>Official opening fixture</title>" }));
  await page.getByRole("button", { name: /載入 YouTube 影片/ }).focus();
  await page.keyboard.press("Enter");
  await expect(page.locator("iframe")).toHaveCount(1);
  await expect(page.locator("iframe")).toHaveAttribute("src", /youtube-nocookie\.com\/embed\/0UmEg8PDV3Y/);
  const context = await browser.newContext({ javaScriptEnabled: false, viewport: { width: 390, height: 844 } });
  try {
    const nativePage = await context.newPage();
    await nativePage.goto(`${e2eOrigin}/catalog/2019/`, { waitUntil: "domcontentloaded" });
...
test("CLIMAX SEASON search preserves monthly ensemble endings, individual singers and official short previews", async ({ page, browser, request }) => {
  test.setTimeout(60_000);
  const slug = "cinderella-girls-climax-2019";
  const mediaRequests: string[] = [];
  page.on("request", (item) => {
    if (/youtube|ytimg|googlevideo/.test(new URL(item.url()).hostname)) mediaRequests.push(item.url());
  });
  ...
  expect(response.status()).toBe(200);
  const detail = await response.json();
  expect(detail.id).toBe("catalog-cinderella-girls-climax-2019");
  expect(detail.themes.map(({ type }: { type: string }) => type)).toEqual(["ED", "ED", "ED"]);
  for (const width of [1280, 960, 390, 320]) {
    await page.setViewportSize({ width, height: 844 });
    await page.goto("/search/", { waitUntil: "domcontentloaded" });
    await page.getByRole("searchbox", { name: "搜尋動畫、歌曲或創作者" }).fill("灰姑娘女孩劇場");
    await page.getByLabel("年份", { exact: true }).selectOption("2019");
    await page.getByLabel("季度", { exact: true }).selectOption("spring");
    await expect(page.locator("[data-catalog-result]")).toHaveCount(1);
    await page.getByRole("link", { name: /アイドルマスター シンデレラガールズ劇場 CLIMAX SEASON/ }).first().click();
    await expect(page).toHaveURL(new RegExp(`/anime/${slug}/$`));
    await expect(page.locator(".anime-hero__zh")).toHaveText("灰姑娘女孩劇場 第四季：CLIMAX SEASON");
    await expect(page.locator(".anime-hero__romaji")).toHaveCount(0);
    await expect(page.locator(".theme-card")).toHaveCount(3);
    await expect(page.locator(".theme-navigation")).toHaveCount(0);
    await expect(page.locator(".theme-card time")).toHaveText(["2019-04-17", "2019-05-22", "2019-06-19"]);
    for (let index = 1; index <= 3; index += 1) {
      const theme = page.locator(`#theme-${slug}-ed-${index}`);
      await expect(theme).toContainText(`TV ${index + 3} 月 ED／CD 完整版；官方試聽短版`);
      await expect(theme.getByRole("button", { name: /載入 YouTube 影片/ })).toHaveCount(1);
    }
    const may = page.locator(`#theme-${slug}-ed-2`);
    await expect(may.locator(".theme-card__credits dd").first()).toHaveText("早見沙織、森下来奈、青木志貴、千菅春香、村中知");
    await may.getByRole("link", { name: "搜尋「森下来奈」的歌曲", exact: true }).focus();
    await page.keyboard.press("Enter");
    await expect(page.getByRole("searchbox", { name: "搜尋動畫、歌曲或創作者" })).toHaveValue("森下来奈");
    for (const [label, value] of [["年份", "2019"], ["季度", "spring"], ["歌曲用途", "ED"]]) await page.getByLabel(label!, { exact: true }).selectOption(value!);
    await expect(page.locator("[data-catalog-result]")).toHaveCount(1);
    await page.getByRole("link", { name: "Max Beat", exact: true }).click();
    await expect(may).toBeFocused();
    await expect(may.locator("details")).not.toHaveAttribute("open", "");
    await may.locator("details summary").focus();
    await page.keyboard.press("Enter");
    await expect(may.locator('a[href="https://columbia.jp/idolmaster/imasnews/190510.html"]')).toBeVisible();
    await expect(page.locator("iframe")).toHaveCount(0);
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  }
  expect(mediaRequests).toEqual([]);
  await page.route("https://www.youtube-nocookie.com/**", (route) => route.fulfill({ contentType: "text/html", body: "<!doctype html><title>Official audio preview fixture</title>" }));
  await page.locator(`#theme-${slug}-ed-2`).getByRole("button", { name: /載入 YouTube 影片/ }).focus();
  await page.keyboard.press("Enter");
  await expect(page.locator("iframe")).toHaveCount(1);
  await expect(page.locator("iframe")).toHaveAttribute("src", /youtube-nocookie\.com\/embed\/bqUcdQSW3qA/);
  await expect(page.locator(`#theme-${slug}-ed-1 iframe, #theme-${slug}-ed-3 iframe`)).toHaveCount(0);
  const context = await browser.newContext({ javaScriptEnabled: false, viewport: { width: 390, height: 844 } });
  try {
    const nativePage = await context.newPage();
    await nativePage.goto(`${e2eOrigin}/catalog/2019/`, { waitUntil: "domcontentloaded" });
    ...
    await nativePage.keyboard.press("Enter");
    await expect(nativePage.locator(`#theme-${slug}-ed-3 a[href="https://columbia.jp/idolmaster/imasnews/190607.html"]`)).toBeVisible();
    await expect(nativePage.locator("iframe")).toHaveCount(0);
    expect(await nativePage.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  } finally {
    await context.close();
...
test("Yo-kai Watch search separates the 2019 series and keeps shared writers and short video choices attached to its songs", async ({ page, browser, request }) => {
  const slug = "youkai-watch-2019";
  const mediaRequests: string[] = [];
  page.on("request", (item) => {
    if (/youtube|ytimg|googlevideo/.test(new URL(item.url()).hostname)) mediaRequests.push(item.url());
  });
  ...
  expect(response.status()).toBe(200);
  const detail = await response.json();
  expect(detail.id).toBe("catalog-youkai-watch-2019");
  expect(detail.themes).toHaveLength(2);
  for (const width of [1280, 960, 390, 320]) {
    await page.setViewportSize({ width, height: 844 });
    await page.goto("/search/");
    await page.getByRole("searchbox", { name: "搜尋動畫、歌曲或創作者" }).fill("妖怪手錶");
    await page.getByLabel("年份", { exact: true }).selectOption("2019");
    await page.getByLabel("季度", { exact: true }).selectOption("spring");
    await expect(page.locator("[data-catalog-result]")).toHaveCount(1);
    await page.getByRole("link", { name: /妖怪ウォッチ！/ }).first().click();
    await expect(page).toHaveURL(new RegExp(`/anime/${slug}/$`));
    await expect(page.locator(".anime-hero__zh")).toHaveText("妖怪手錶！");
    await expect(page.locator(".anime-hero__romaji")).toHaveCount(0);
    await expect(page.locator(".theme-card")).toHaveCount(2);
    await expect(page.locator(".theme-navigation")).toHaveCount(0);
    await expect(page.locator(".theme-card time")).toHaveText(["2019-06-05", "2019-06-05"]);
    const opening = page.locator(`#theme-${slug}-op-1`);
    const ending = page.locator(`#theme-${slug}-ed-1`);
    await expect(opening.locator(".theme-card__credits dd")).toHaveText(["高木貴司", "紘毅", "菊谷知樹"]);
    await expect(ending.locator(".theme-card__credits dd")).toHaveText(["ラッキィ池田、高木貴司", "菊谷知樹", "菊谷知樹"]);
    await ending.getByRole("link", { name: "搜尋「ラッキィ池田」的歌曲", exact: true }).focus();
    await page.keyboard.press("Enter");
    await expect(page.getByRole("searchbox", { name: "搜尋動畫、歌曲或創作者" })).toHaveValue("ラッキィ池田");
    for (const [label, value] of [["年份", "2019"], ["季度", "spring"], ["歌曲用途", "ED"]]) await page.getByLabel(label!, { exact: true }).selectOption(value!);
    await expect(page.locator("[data-catalog-result]")).toHaveCount(1);
    await page.getByRole("link", { name: "ようかい体操第一 ～つづき～", exact: true }).click();
    await expect(ending).toBeFocused();
    await expect(opening).toContainText("MV 短版");
    await expect(ending).toContainText("振付影片為短版");
    await expect(ending.getByRole("button", { name: /載入 YouTube 影片/ })).toHaveCount(2);
    await expect(ending.getByRole("button", { name: /振りビデオ\(short ver\.\)/ })).toBeVisible();
    await expect(page.locator("iframe")).toHaveCount(0);
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  }
  expect(mediaRequests).toEqual([]);
  await page.route("https://www.youtube-nocookie.com/**", (route) => route.fulfill({ contentType: "text/html", body: "<!doctype html><title>Official short dance video fixture</title>" }));
  const dance = page.locator(`#theme-${slug}-ed-1`).getByRole("button", { name: /振りビデオ\(short ver\.\)/ });
  await dance.focus();
  await page.keyboard.press("Enter");
  await expect(page.locator("iframe")).toHaveCount(1);
  await expect(page.locator("iframe")).toHaveAttribute("src", /youtube-nocookie\.com\/embed\/BOiLAwterTg/);
  await expect(page.locator(`#theme-${slug}-op-1 iframe`)).toHaveCount(0);
  ...
  try {
    ...
    await nativePage.keyboard.press("Enter");
    await expect(sources.locator('a[href="https://popholic.jp/archives/12212"]')).toBeVisible();
    await expect(nativePage.locator("iframe")).toHaveCount(0);
    expect(await nativePage.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  } finally {
    await context.close();
...
test("weekday navigation follows populated groups and remains useful after filters and without JavaScript", async ({ page, browser }) => {
  for (const width of [1280, 390]) {
    ...
    await page.getByRole("checkbox", { name: "有正版影片" }).check();
    await expect(navigation.getByRole("link")).toHaveText(["週一", "週二", "週三", "週四", "週五", "週六", "週日"]);
    await expect(page.locator("[data-result-count]")).toHaveText("24");
    const expectedCounts = [3, 5, 1, 2, 4, 4, 5];
  ...
  await expect(nativePage.locator("[data-anime-card]")).toHaveCount(39);
