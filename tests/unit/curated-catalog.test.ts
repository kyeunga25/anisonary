...
describe("curated public catalogue", () => {
  it("publishes all twenty-nine reviewed seasonal snapshots", () => {
    ...
    expect(curatedSeasonDetails.map((season) => season.anime.length)).toEqual([70, 70, 66, 75, 82, 59, 88, 68, 76, 75, 100, 75, 79, 72, 80, 69, 79, 58, 65, 46, 73, 67, 67, 31, 61, 58, 67, 42, 30]);
    ...
    expect(curatedAnimeDetails).toHaveLength(1947);
    expect(curatedAnimeDetails.filter((anime) => anime.themes.length > 0)).toHaveLength(1583);
    expect(curatedAnimeDetails.flatMap((anime) => anime.themes)).toHaveLength(4307);
    expect(curatedAnimeDetails.filter((anime) => anime.themeAvailability === "documented")).toHaveLength(1583);
