...
describe("curated public catalogue", () => {
  it("publishes all twenty-nine reviewed seasonal snapshots", () => {
    ...
    expect(curatedSeasonDetails.map((season) => season.anime.length)).toEqual([70, 70, 66, 75, 82, 59, 88, 68, 76, 75, 100, 75, 79, 72, 80, 69, 79, 58, 65, 46, 73, 67, 67, 31, 61, 58, 67, 42, 39]);
    ...
    expect(curatedAnimeDetails).toHaveLength(1956);
    expect(curatedAnimeDetails.filter((anime) => anime.themes.length > 0)).toHaveLength(1591);
    expect(curatedAnimeDetails.flatMap((anime) => anime.themes)).toHaveLength(4348);
    expect(curatedAnimeDetails.filter((anime) => anime.themeAvailability === "documented")).toHaveLength(1591);
    expect(curatedAnimeDetails.filter((anime) => anime.themeAvailability === "not_used")).toHaveLength(2);
    expect(curatedAnimeDetails.filter((anime) => anime.themeAvailability === "not_announced")).toHaveLength(363);
    ...
    expect(youtubeLinks).toHaveLength(567);
  ...
  it("keeps the reviewed 2020 fall serial boundary and separates OP/ED from insert songs", () => {
    const fall = curatedSeasonDetails.find((season) => season.id === "2020-fall");
    expect(fall?.anime).toHaveLength(67);
    const fallSeeds = curatedAnimeSeeds.filter((seed) => seed.seasonIds.includes("2020-fall"));
    ...
    expect(fallSeeds.flatMap(({ themes }) => themes)).toHaveLength(180);

    for (const seed of fallSeeds) {
      expect(seed.startDate >= "2020-09-28" && seed.startDate <= "2020-12-31").toBe(true);
      expect(seed.seasonIds[0]).toBe("2020-fall");
      expect(seed.verifiedAt).toBe("2026-09-02");
      const editorialWeekday = new Date(`${seed.startDate}T00:00:00Z`).getUTCDay() || 7;
      expect(seed.editorialWeekday, `${seed.anilistId} weekday`).toBe(editorialWeekday);
      expect(seed.titleJa.trim()).not.toBe("");
    ...
    for (const includedAniListId of [122632, 114446, 113415, 113970, 110277, 179778, 136642]) {
      expect(fallIds.has(`curated-${includedAniListId}`)).toBe(true);
    }
    expect(fallIds.has("curated-113585")).toBe(false);

    const fallDetails = curatedAnimeDetails.filter((anime) => fallIds.has(anime.id));
    expect(fallDetails.filter((anime) => anime.themeAvailability === "not_announced")
      .map(({ id }) => Number(id.replace("curated-", ""))).sort((left, right) => left - right)).toEqual(
        [114721, 121799, 123474, 125491, 126271, 126506, 128019, 147764, 187055]
      );
    expect(fallDetails.flatMap((anime) => anime.themes)).toHaveLength(180);
    ...
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-122632")?.themes
      .map(({ type, titleJa }) => [type, titleJa])).toEqual([
        ["OP", "カ！カ！カ！カミズモード！"],
        ["ED", "笑顔でBYE！～カミズモ音頭～"]
      ]);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-125887")?.themes
      .map(({ type, titleJa }) => [type, titleJa])).toEqual([
        ["OP", "HERO READY GO パトレイサー！"]
      ]);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-121567")?.themes
      .map(({ type, titleJa }) => [type, titleJa])).toEqual([
        ["OP", "終末への天撃"],
        ["ED", "笑顔ヶ丘音頭"],
        ["ED", "私立笑顔ヶ丘中学校卒業歌"]
      ]);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-117086")?.themes
      .map(({ titleJa }) => titleJa)).toEqual(["好日", "黒ビィんの翼", "ティアマト・マグナ"]);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-105244")?.themes
      .find(({ titleJa }) => titleJa === "月ノ詩。")).toMatchObject({
        type: "ED",
        sequence: 13,
        artistDisplayName: "Six Gravity＆Procellarum"
      });
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-116700")?.themes
      .map(({ titleJa }) => titleJa)).toEqual(["にゃんだーわんだーデイズ", "Lovely Days"]);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-113415")?.themes
      .flatMap(({ videos }) => videos.map(({ youtubeVideoId }) => youtubeVideoId))).toEqual([
        "v8bZVdTgXoY",
        "8nNujr378EA",
        "AWEm4tA2hMc",
        "u9rT73uzVKI"
      ]);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-110277")?.themes
      .flatMap(({ videos }) => videos.map(({ youtubeVideoId }) => youtubeVideoId))).toEqual([
        "6TolbTZXDjI",
        "AZl8UDsqsoM"
      ]);

    for (const theme of fallDetails.flatMap(({ themes }) => themes)) {
      expect(theme.sources.some(({ role }) => role === "first_party"), theme.id).toBe(true);
      expect(theme.sources.some(({ role }) => role === "cross_check"), theme.id).toBe(true);
    }
  });

  it("keeps the reviewed 2020 summer first-release boundary and rejects unproven OP/ED placements", () => {
    const summer = curatedSeasonDetails.find((season) => season.id === "2020-summer");
    expect(summer?.anime).toHaveLength(31);
    const summerSeeds = curatedAnimeSeeds.filter((seed) => seed.seasonIds.includes("2020-summer"));
    expect(summerSeeds).toHaveLength(31);
    expect(new Set(summerSeeds.map(({ anilistId }) => anilistId)).size).toBe(31);
    expect(new Set(summerSeeds.map(({ slug }) => slug)).size).toBe(31);
    expect(summerSeeds.flatMap(({ themes }) => themes)).toHaveLength(61);

    for (const seed of summerSeeds) {
      expect(seed.startDate >= "2020-07-01" && seed.startDate <= "2020-09-17").toBe(true);
      expect(seed.seasonIds[0]).toBe("2020-summer");
      expect(seed.verifiedAt).toBe("2026-09-02");
      const editorialWeekday = new Date(`${seed.startDate}T00:00:00Z`).getUTCDay() || 7;
      expect(seed.editorialWeekday, `${seed.anilistId} weekday`).toBe(editorialWeekday);
      expect(seed.titleJa.trim()).not.toBe("");
    ...
    for (const includedAniListId of [116259, 104937, 114236, 112357, 119696, 120046, 114943, 120325]) {
      expect(summerIds.has(`curated-${includedAniListId}`)).toBe(true);
    }
    expect(summerIds.has("curated-110349")).toBe(false);

    const summerDetails = curatedAnimeDetails.filter((anime) => summerIds.has(anime.id));
    expect(summerDetails.filter((anime) => anime.themeAvailability === "not_announced")
      .map(({ id }) => Number(id.replace("curated-", ""))).sort((left, right) => left - right)).toEqual(
        [108356, 110857, 117150, 121684, 122248]
      );
    expect(summerDetails.flatMap((anime) => anime.themes)).toHaveLength(61);
    expect(summerDetails.flatMap((anime) => anime.themes).flatMap((theme) => theme.videos)).toHaveLength(52);
    expect(new Set(summerDetails.flatMap((anime) => anime.themes)
      .flatMap((theme) => theme.videos.map(({ youtubeVideoId }) => youtubeVideoId))).size).toBe(52);

    expect(curatedAnimeDetails.find(({ id }) => id === "curated-110857")?.themes).toEqual([]);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-121684")?.themes).toEqual([]);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-114943")?.themes
      .map(({ type, sequence, titleJa }) => [type, sequence, titleJa])).toEqual([
        ["ED", 1, "BREAK THE CHAIN"]
      ]);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-104937")?.themes
      .flatMap(({ videos }) => videos.map(({ youtubeVideoId }) => youtubeVideoId))).toEqual([
        "_qGD3hDAdS0",
        "MKoh5jdNiGU",
        "lCMPUzgiETI",
    ...
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-114236")?.themes
      .map(({ type, titleJa }) => [type, titleJa])).toEqual([
        ["OP", "SPARK-AGAIN"],
        ["OP", "Torch of Liberty"],
        ["ED", "ID"],
    ...
    for (const theme of summerDetails.flatMap(({ themes }) => themes)) {
      expect(theme.sources.some(({ role }) => role === "first_party"), theme.id).toBe(true);
      expect(theme.sources.some(({ role }) => role === "cross_check"), theme.id).toBe(true);
    }
  });

  it("keeps the reviewed 2020 spring boundary and separates themes from insert songs", () => {
    const spring = curatedSeasonDetails.find((season) => season.id === "2020-spring");
    expect(spring?.anime).toHaveLength(61);
    const springSeeds = curatedAnimeSeeds.filter((seed) => seed.seasonIds.includes("2020-spring"));
    expect(springSeeds).toHaveLength(61);
    expect(new Set(springSeeds.map(({ anilistId }) => anilistId)).size).toBe(61);
    expect(new Set(springSeeds.map(({ slug }) => slug)).size).toBe(61);
    expect(springSeeds.flatMap(({ themes }) => themes)).toHaveLength(208);

    for (const seed of springSeeds) {
      expect(seed.startDate >= "2020-04-01" && seed.startDate <= "2020-06-04").toBe(true);
      expect(seed.seasonIds[0]).toBe("2020-spring");
      expect(seed.verifiedAt).toBe("2026-09-02");
      const editorialWeekday = new Date(`${seed.startDate}T00:00:00Z`).getUTCDay() || 7;
      expect(seed.editorialWeekday, `${seed.anilistId} weekday`).toBe(editorialWeekday);
      expect(seed.titleJa.trim()).not.toBe("");
    ...
    for (const includedAniListId of [110130, 114072, 117096, 111762, 110349, 108522]) {
      expect(springIds.has(`curated-${includedAniListId}`)).toBe(true);
    }
    for (const excludedAniListId of [116259, 110354]) {
      expect(springIds.has(`curated-${excludedAniListId}`)).toBe(false);
    }

    const springDetails = curatedAnimeDetails.filter((anime) => springIds.has(anime.id));
    expect(springDetails.filter((anime) => anime.themeAvailability === "not_announced")
      .map(({ id }) => Number(id.replace("curated-", ""))).sort((left, right) => left - right)).toEqual(
        [116320, 116789]
      );
    expect(springDetails.flatMap((anime) => anime.themes)).toHaveLength(208);
    expect(springDetails.flatMap((anime) => anime.themes).flatMap((theme) => theme.videos)).toHaveLength(102);
    expect(new Set(springDetails.flatMap((anime) => anime.themes)
      .flatMap((theme) => theme.videos.map(({ youtubeVideoId }) => youtubeVideoId))).size).toBe(102);

    expect(curatedAnimeDetails.find(({ id }) => id === "curated-113906")?.themes[0]?.videos)
      .toEqual([expect.objectContaining({ youtubeVideoId: "Veb7GRKI5rM", officialStatus: "official" })]);

    expect(curatedAnimeDetails.find(({ id }) => id === "curated-112353")?.themes
      .map(({ type, titleJa }) => [type, titleJa])).toEqual([
        ["OP", "aranami"],
        ["ED", "Pride"]
      ]);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-110458")?.themes
      .map(({ type, titleJa }) => [type, titleJa])).toEqual([
        ["OP", "天秤-Libra-"],
        ["ED", "through the dark"]
      ]);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-114072")?.themes
      .map(({ type, sequence, titleJa }) => [type, sequence, titleJa])).toEqual([
        ["OP", 1, "ナナナナナナナ"],
        ["OP", 2, "ハレヴタイ"],
        ["ED", 1, "ゴーハ第7小学校校歌"],
        ["ED", 2, "ミニスケープ"],
        ["ED", 3, "ゴーハ第7小学校校歌（ルーク ver.）"],
        ["ED", 4, "ゴーハ第7小学校校歌（4人合唱 ver.）"],
        ["ED", 5, "ゴーハ第7小学校校歌（遊我 ver.）"],
        ["ED", 6, "ゴーハ第7小学校校歌（ガクト ver.）"],
        ["ED", 7, "ゴーハ第7小学校校歌（ロミン ver.）"],
        ["ED", 8, "Never Looking Back"]
      ]);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-114072")?.themes
      .some(({ titleJa }) => titleJa === "Are You Re:D?")).toBe(false);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-116789")?.themes).toEqual([]);
  ...
  it("keeps the reviewed 2019 fall boundary and preserves exact OP/ED variants", () => {
    const fall = curatedSeasonDetails.find((season) => season.id === "2019-fall");
    expect(fall?.anime).toHaveLength(67);
    const fallSeeds = curatedAnimeSeeds.filter((seed) => seed.seasonIds.includes("2019-fall"));
    expect(fallSeeds).toHaveLength(67);
    expect(new Set(fallSeeds.map(({ anilistId }) => anilistId)).size).toBe(67);
    expect(new Set(fallSeeds.map(({ slug }) => slug)).size).toBe(67);
    ...
    for (const seed of fallSeeds) {
      ...
      expect(seed.seasonIds[0]).toBe("2019-fall");
      expect(seed.verifiedAt).toBe("2026-09-02");
      const editorialWeekday = new Date(`${seed.startDate}T00:00:00Z`).getUTCDay() || 7;
      expect(seed.editorialWeekday, `${seed.anilistId} weekday`).toBe(editorialWeekday);
      expect(seed.titleJa.trim()).not.toBe("");
    ...
    const fallIds = new Set(fall?.anime.map((anime) => anime.id));
    for (const includedAniListId of [110382, 100780, 112036, 101239, 108759, 112153, 111964]) {
      expect(fallIds.has(`curated-${includedAniListId}`)).toBe(true);
    }
    for (const excludedAniListId of [115908, 112455, 113221, 113654, 113828, 154175, 112625]) {
    ...
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-105156")?.themes
      .map(({ type, titleJa }) => [type, titleJa])).toEqual([
        ["OP", "TIT FOR TAT"],
        ["ED", "be perfect, plz!"],
        ["ED", "さいしょからずっと"]
      ]);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-108891")?.themes
      .map(({ type, titleJa }) => [type, titleJa])).toEqual([
        ["OP", "Harmony"],
        ["ED", "Rainbow"]
      ]);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-108891")?.themes
      .some(({ titleJa }) => titleJa === "Tone" || titleJa === "Speechless")).toBe(false);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-101349")?.themes
      .map(({ type, titleJa }) => [type, titleJa])).toEqual([
        ["ED", "Live and let die"],
        ["ED", "イノチ食ム魂"],
        ["ED", "イノチ食ム魂（requiem ver.）"],
        ["ED", "The next new world that no one knows（blood stained ver.）"]
      ]);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-109616")?.themes
      .map(({ type, titleJa }) => [type, titleJa])).toEqual([["OP", "SURVIVE OF VISION"]]);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-108759")?.themes
      .map(({ type, titleJa }) => [type, titleJa])).toEqual([
        ["OP", "Resolution"],
        ["ED", "unlasting"]
      ]);
    ...
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-112153")?.themes
      .map(({ type, titleJa }) => ({ type, titleJa }))).toEqual([
        { type: "OP", titleJa: "1・2・3" },
        { type: "OP", titleJa: "1・2・3" },
        { type: "OP", titleJa: "1・2・3" },
        { type: "OP", titleJa: "1・2・3" },
        { type: "ED", titleJa: "ポケモンしりとり（ピカチュウ→ミュウVer.）" },
        { type: "ED", titleJa: "ポケモンしりとり（ミュウ→ザマゼンタVer.）" },
        { type: "ED", titleJa: "バツグンタイプ" },
        { type: "ED", titleJa: "1・2・3" }
      ]);
  ...
  it("keeps the reviewed 2021 spring serial boundary and official OP/ED identities conservative", () => {
    const spring = curatedSeasonDetails.find((season) => season.id === "2021-spring");
    expect(spring?.anime).toHaveLength(73);
    const springSeeds = curatedAnimeSeeds.filter((seed) => seed.seasonIds.includes("2021-spring"));
    expect(springSeeds).toHaveLength(73);
    expect(new Set(springSeeds.map(({ anilistId }) => anilistId)).size).toBe(73);
    expect(new Set(springSeeds.map(({ slug }) => slug)).size).toBe(73);
    expect(springSeeds.flatMap(({ themes }) => themes)).toHaveLength(131);

    for (const seed of springSeeds) {
      expect(seed.startDate).toMatch(/^2021-(?:03|04|05|06)-\d{2}$/);
      expect(seed.startDate >= "2021-03-22" && seed.startDate <= "2021-06-17").toBe(true);
      expect(seed.seasonIds[0]).toBe("2021-spring");
      expect(seed.verifiedAt).toBe("2026-09-02");
      const editorialWeekday = new Date(`${seed.startDate}T00:00:00Z`).getUTCDay() || 7;
      expect(seed.editorialWeekday, `${seed.anilistId} weekday`).toBe(editorialWeekday);
      expect(seed.titleJa.trim()).not.toBe("");
    ...
    const springIds = new Set(spring?.anime.map((anime) => anime.id));
    for (const includedAniListId of [124786, 117193, 119675, 128546, 110733, 120376, 128737, 101216]) {
      expect(springIds.has(`curated-${includedAniListId}`)).toBe(true);
    }
    for (const excludedAniListId of [133849, 136135, 137333, 113653, 124115, 131773]) {
      expect(springIds.has(`curated-${excludedAniListId}`)).toBe(false);
    }

    const springDetails = curatedAnimeDetails.filter((anime) => springIds.has(anime.id));
    expect(springDetails.filter((anime) => anime.themeAvailability === "not_announced")).toHaveLength(22);
    expect(springDetails.flatMap((anime) => anime.themes)).toHaveLength(131);
    expect(springDetails.flatMap((anime) => anime.themes).flatMap((theme) => theme.videos)).toHaveLength(66);

    expect(curatedAnimeDetails.find(({ id }) => id === "curated-128546")?.themes
      .map(({ type, titleJa }) => [type, titleJa])).toEqual([["OP", "Sing My Pleasure"]]);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-110733")?.themes
      .map(({ type, titleJa }) => [type, titleJa])).toEqual([
        ["OP", "大河よ共に泣いてくれ"],
        ["ED", "夢を手に、戻れる場所もない日々を"]
      ]);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-124675")?.themes
      .map(({ type, titleJa }) => [type, titleJa])).toEqual([
        ["OP", "Chance! & Revenge!"],
        ["ED", "戦略的で予測不能なラブコメディのエンディング曲"]
      ]);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-124675")?.themes
      .some(({ titleJa }) => titleJa === "パラダイスSOS")).toBe(false);

    expect(curatedAnimeDetails.find(({ id }) => id === "curated-116741")?.themes
      .find(({ titleJa }) => titleJa === "カモナ・テンペスト！")?.artistDisplayName).toBe(
        "リムル（CV：岡咲美保）、大賢者（CV：豊口めぐみ）、ヴェルドラ（CV：前野智昭）、シュナ（CV：千本木彩花）、シオン（CV：M・A・O）、ランガ（CV：小林親弘）、ゴブタ（CV：泊 明日菜）"
      );
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-101216")?.themes
      .find(({ type }) => type === "ED")).toMatchObject({
        titleJa: "blue moment（にゃ～たん＆なぁ～たん ver.）",
        artistDisplayName: "にゃ～たん（CV：村川梨衣）＆なぁ～たん（CV：鈴木愛奈）"
      });
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-120376")?.themes
      .map(({ type, titleJa }) => [type, titleJa])).toEqual([
        ["OP", "Calling -Animation OP ver.-"],
        ["OP", "Twister -Animation OP ver.-"],
        ["ED", "カルペディエム"]
      ]);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-128737")?.themes
      .map(({ type, titleJa }) => [type, titleJa])).toEqual([
        ["OP", "ニューチャレンジャー"],
        ["ED", "キズナ・レール"],
        ["ED", "駆け上がるボルテージ"],
  ...
  it("keeps the reviewed 2021 summer serial boundary and complex OP/ED identities conservative", () => {
    const summer = curatedSeasonDetails.find((season) => season.id === "2021-summer");
    expect(summer?.anime).toHaveLength(46);
    const summerSeeds = curatedAnimeSeeds.filter((seed) => seed.seasonIds.includes("2021-summer"));
    expect(summerSeeds).toHaveLength(46);
    expect(new Set(summerSeeds.map(({ anilistId }) => anilistId)).size).toBe(46);
    expect(new Set(summerSeeds.map(({ slug }) => slug)).size).toBe(46);
    expect(summerSeeds.flatMap(({ themes }) => themes)).toHaveLength(104);

    for (const seed of summerSeeds) {
      expect(seed.startDate).toMatch(/^2021-(?:07|08)-\d{2}$/);
      expect(seed.startDate >= "2021-07-01" && seed.startDate <= "2021-08-15").toBe(true);
      expect(seed.seasonIds[0]).toBe("2021-summer");
      expect(seed.verifiedAt).toBe("2026-09-02");
      const editorialWeekday = new Date(`${seed.startDate}T00:00:00Z`).getUTCDay() || 7;
      expect(seed.editorialWeekday, `${seed.anilistId} weekday`).toBe(editorialWeekday);
      expect(seed.titleJa.trim()).not.toBe("");
    ...
    const summerIds = new Set(summer?.anime.map((anime) => anime.id));
    for (const includedAniListId of [131149, 107717, 114979, 124494, 128703, 122052, 136517, 180850]) {
      expect(summerIds.has(`curated-${includedAniListId}`)).toBe(true);
    }
    for (const excludedAniListId of [135866, 136206, 136946, 137102, 139342, 139756, 140099, 148002]) {
      expect(summerIds.has(`curated-${excludedAniListId}`)).toBe(false);
    }

    const summerDetails = curatedAnimeDetails.filter((anime) => summerIds.has(anime.id));
    expect(summerDetails.filter((anime) => anime.themeAvailability === "not_announced")).toHaveLength(8);
    expect(summerDetails.flatMap((anime) => anime.themes)).toHaveLength(104);
    expect(summerDetails.flatMap((anime) => anime.themes).flatMap((theme) => theme.videos)).toHaveLength(65);

    expect(
      curatedAnimeDetails.find(({ id }) => id === "curated-114979")?.themes.map(({ type, titleJa }) => [type, titleJa])
    ).toEqual([
      ["OP", "START!! True dreams"],
      ["ED", "未来は風のように"]
    ]);
    expect(
      curatedAnimeDetails.find(({ id }) => id === "curated-127721")?.themes.map(({ type, titleJa }) => [type, titleJa])
    ).toEqual([
      ["OP", "THE POLiCY"],
      ["ED", "PLACES"]
    ]);
    expect(
      curatedAnimeDetails.find(({ id }) => id === "curated-131880")?.themes.map(({ type, titleJa }) => [type, titleJa])
    ).toEqual([["ED", "It's My Party"]]);
    expect(
      curatedAnimeDetails.find(({ id }) => id === "curated-125206")?.themes.map(({ type, titleJa }) => [type, titleJa])
    ).toEqual([
      ["OP", "ギャンブル"],
      ["ED", "ああ人生に涙あり"],
      ["ED", "ビューティフル・ドリーマー"]
    ...
    const kagekiThemes = curatedAnimeDetails.find(({ id }) => id === "curated-125446")?.themes ?? [];
    expect(kagekiThemes).toHaveLength(6);
    expect(
      kagekiThemes.filter(({ titleJa }) => titleJa === "薔薇と私").map(({ artistDisplayName }) => artistDisplayName)
    ).toEqual(["星野 薫（CV：大地 葉）", "沢田千夏（CV：松田利冴）×沢田千秋（CV：松田颯水）"]);

    const tsukiproThemes = curatedAnimeDetails.find(({ id }) => id === "curated-125909")?.themes ?? [];
    expect(tsukiproThemes).toHaveLength(17);
    expect(tsukiproThemes.find(({ type, sequence }) => type === "ED" && sequence === 13)?.titleJa).toBe("Best Wishes,");
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-128703")?.themes[0]).toMatchObject({
      titleJa: "きゃんとすとっぷ・ふるーてぃー",
      versionLabel: "主題歌（官方分類）"
    });

    const publishedSummerThemeTitles = summerDetails.flatMap(({ themes }) => themes).map(({ titleJa }) => titleJa);
    for (const excludedTitle of [
      ...
      expect(publishedSummerThemeTitles).not.toContain(excludedTitle);
    }

    for (const theme of summerDetails.flatMap(({ themes }) => themes)) {
      expect(
        theme.sources.some(({ role }) => role === "first_party"),
        theme.id
      ).toBe(true);
      expect(
        theme.sources.some(({ role }) => role === "cross_check"),
        theme.id
      ).toBe(true);
    }
  });
  ...
  it("keeps the reviewed 2022 winter boundary and official OP/ED identities conservative", () => {
    const winter = curatedSeasonDetails.find((season) => season.id === "2022-winter");
    expect(winter?.anime).toHaveLength(58);
    const winterSeeds = curatedAnimeSeeds.filter((seed) => seed.seasonIds.includes("2022-winter"));
    expect(winterSeeds).toHaveLength(58);
    expect(new Set(winterSeeds.map(({ anilistId }) => anilistId)).size).toBe(58);
    expect(new Set(winterSeeds.map(({ slug }) => slug)).size).toBe(58);
    expect(winterSeeds.flatMap(({ themes }) => themes)).toHaveLength(111);

    for (const seed of winterSeeds) {
      expect(seed.startDate).toMatch(/^(?:2021-12|2022-(?:01|02|03))-\d{2}$/);
      expect(seed.startDate >= "2021-12-01" && seed.startDate <= "2022-03-19").toBe(true);
      expect(seed.seasonIds[0]).toBe("2022-winter");
      expect(seed.verifiedAt).toBe("2026-09-01");
      const editorialWeekday = new Date(`${seed.startDate}T00:00:00Z`).getUTCDay() || 7;
      expect(seed.editorialWeekday, `${seed.anilistId} weekday`).toBe(editorialWeekday);
      expect(seed.titleJa.trim()).not.toBe("");
      expect((seed.titleZhHant ?? "").trim()).not.toBe("");
    ...
    const winterIds = new Set(winter?.anime.map((anime) => anime.id));
    for (const includedAniListId of [131942, 130389, 140700, 118465, 101705, 142794, 139589, 146503]) {
      expect(winterIds.has(`curated-${includedAniListId}`)).toBe(true);
    }
    for (const excludedAniListId of [142329, 189717, 146004, 143612, 151985, 154041]) {
      expect(winterIds.has(`curated-${excludedAniListId}`)).toBe(false);
    }
    expect(curatedAnimeDetails.filter((anime) => (
      winterIds.has(anime.id) && anime.themeAvailability === "not_announced"
    ))).toHaveLength(11);
    expect(curatedAnimeDetails.filter((anime) => winterIds.has(anime.id))
      .flatMap((anime) => anime.themes)
      .flatMap((theme) => theme.videos)).toHaveLength(26);

    const jojo = curatedAnimeDetails.find(({ id }) => id === "curated-131942");
    expect(jojo?.themes.map(({ type, sequence, titleJa }) => [type, sequence, titleJa])).toEqual([
      ["OP", 1, "STONE OCEAN"],
      ["ED", 1, "Distant Dreamer"]
    ]);
    const takagi = curatedAnimeDetails.find(({ id }) => id === "curated-138424");
    expect(takagi?.themes).toHaveLength(9);
    expect(takagi?.themes.flatMap(({ videos }) => videos)).toHaveLength(9);
    expect(takagi?.themes.some(({ titleJa }) => titleJa === "まっすぐ～ラララver.～")).toBe(false);
    const cue = curatedAnimeDetails.find(({ id }) => id === "curated-125682");
    expect(cue?.themes.map(({ titleJa }) => titleJa)).toEqual([
      "スタートライン", "Tomorrow's Diary", "はじまりの鐘の音が鳴り響く空",
      "さよならレディーメイド", "ゆめだより", "My Sustainer", "ミライキャンバス"
    ]);

    for (const [animeId, themeTitle] of [
      ["curated-130389", "Ripe Aster"],
      ["curated-143415", "こつこつ世界征服"],
      ["curated-101705", "Oarana"],
      ["curated-137378", "ひとひらの未来"]
    ] as const) {
      const theme = curatedAnimeDetails.find(({ id }) => id === animeId)?.themes
        .find(({ titleJa }) => titleJa === themeTitle);
      expect(theme?.versionLabel, animeId).toBe("主題歌（官方分類）");
      expect(theme?.sources.some(({ role }) => role === "first_party"), animeId).toBe(true);
      expect(theme?.sources.some(({ role }) => role === "cross_check"), animeId).toBe(true);
    }

    const garugaku = curatedAnimeDetails.find(({ id }) => id === "curated-142216");
    expect(garugaku?.themes.map(({ titleJa }) => titleJa)).toEqual([
      "ichigo ～一期一会～",
      "3・2・1…BREAK!"
    ]);
  });

  it("keeps the reviewed 2022 spring serial boundary and complex OP/ED identities conservative", () => {
    const spring = curatedSeasonDetails.find((season) => season.id === "2022-spring");
    expect(spring?.anime).toHaveLength(79);
    const springSeeds = curatedAnimeSeeds.filter((seed) => seed.seasonIds.includes("2022-spring"));
    ...
    expect(springSeeds.flatMap(({ themes }) => themes)).toHaveLength(150);

    for (const seed of springSeeds) {
      expect(seed.startDate).toMatch(/^2022-(?:03|04|05|06)-\d{2}$/);
      expect(seed.startDate >= "2022-03-01" && seed.startDate <= "2022-06-28").toBe(true);
      expect(seed.seasonIds[0]).toBe("2022-spring");
      expect(seed.verifiedAt).toBe("2026-09-01");
      const editorialWeekday = new Date(`${seed.startDate}T00:00:00Z`).getUTCDay() || 7;
      expect(seed.editorialWeekday, `${seed.anilistId} weekday`).toBe(editorialWeekday);
      expect(seed.titleJa.trim()).not.toBe("");
    ...
    const springIds = new Set(spring?.anime.map((anime) => anime.id));
    for (const includedAniListId of [143289, 124032, 125447, 141774, 140960, 141350, 148819, 108353]) {
      expect(springIds.has(`curated-${includedAniListId}`)).toBe(true);
    }
    expect(curatedAnimeDetails.filter((anime) => (
    ...
    const reviewedMusic = new Map([
      ["curated-140960", ["ミックスナッツ", "喜劇"]],
      ["curated-141774", ["チキチキバンバン", "気分上々↑↑", "六本木うどん屋（仮）", "気分上々↑↑", "気分上々↑↑"]],
      ["curated-125367", ["GIRI GIRI", "ハートはお手上げ", "My Nonfiction"]],
      ["curated-129201", ["星が泳ぐ", "夏夢ノイジー", "回夏", "失恋ソング沢山聴いて 泣いてばかりの私はもう。", "失恋ソング沢山聴いて 泣いてばかりの私はもう。（acoustic ver.）"]],
      ["curated-134732", ["無心拍数", "Presence", "Blue Diary", "カラー・リリィの恋文"]],
      ["curated-136080", ["ケアレス", "Lapis", "オルゴール"]]
    ]);
    ...
    const spyFamily = curatedAnimeDetails.find(({ id }) => id === "curated-140960");
    expect(spyFamily?.themes.flatMap(({ videos }) => videos).map(({ youtubeVideoId }) => youtubeVideoId))
      .toEqual(["U_rWZK_8vUY", "nRKJBpFFsuI"]);
    const kunoichi = curatedAnimeDetails.find(({ id }) => id === "curated-141350");
    expect(kunoichi?.themes).toHaveLength(14);
    expect(kunoichi?.themes.slice(1, 3).map(({ titleJa }) => titleJa)).toEqual([
      "あかね組活動日誌 ～戌班～",
      "あかね組活動日誌 ～未班～"
    ]);
    expect(kunoichi?.themes.at(-1)?.artistDisplayName).toContain("リンドウ");

    const nanaLucky = curatedAnimeDetails.find(({ id }) => id === "curated-148819");
    expect(nanaLucky?.themes[0]).toMatchObject({
      type: "OP",
      sequence: 1,
      titleJa: "Lucky Boy, Lucky Girl",
      artistDisplayName: "呂布カルマ",
      versionLabel: "主題歌（官方分類）"
    });
    expect(nanaLucky?.themes[0]?.sources).toContainEqual(expect.objectContaining({
      role: "first_party",
      url: expect.stringContaining("izanagigames.co.jp")
    }));
  });
  ...
  it("keeps the reviewed 2023 winter serial boundary and multi-ending identities conservative", () => {
    const winter = curatedSeasonDetails.find((season) => season.id === "2023-winter");
    expect(winter?.anime).toHaveLength(72);
    const winterSeeds = curatedAnimeSeeds.filter((seed) => seed.seasonIds.includes("2023-winter"));
    expect(winterSeeds).toHaveLength(72);
    expect(new Set(winterSeeds.map(({ anilistId }) => anilistId)).size).toBe(72);
    expect(new Set(winterSeeds.map(({ slug }) => slug)).size).toBe(72);
    expect(winterSeeds.flatMap(({ themes }) => themes)).toHaveLength(159);

    for (const seed of winterSeeds) {
      expect(seed.startDate).toMatch(/^2023-(?:01|02|03)-\d{2}$/);
      expect(seed.startDate >= "2023-01-01" && seed.startDate <= "2023-03-28").toBe(true);
      const editorialWeekday = new Date(`${seed.startDate}T00:00:00Z`).getUTCDay() || 7;
      expect(seed.editorialWeekday, `${seed.anilistId} weekday`).toBe(editorialWeekday);
      expect(seed.titleJa.trim()).not.toBe("");
      expect((seed.titleZhHant ?? "").trim()).not.toBe("");
    ...
    const winterIds = new Set(winter?.anime.map((anime) => anime.id));
    for (const excludedAniListId of [
      156059, 160836, 154924, 160865, 161923, 161505, 159731, 159726,
      159727, 159728, 159730, 176881, 184757, 160088, 169542
    ]) {
      expect(winterIds.has(`curated-${excludedAniListId}`)).toBe(false);
    }
    expect(curatedAnimeDetails.filter((anime) => (
    ...
    expect(curatedAnimeDetails.filter((anime) => winterIds.has(anime.id))
      .flatMap((anime) => anime.themes)
      .flatMap((theme) => theme.videos)).toHaveLength(7);

    const reviewedMusic = new Map([
      ["curated-154982", ["シュフノミチ", "極・夫婦街道"]],
      ["curated-150653", ["Paranoid", "云う透り"]],
      ["curated-159569", ["ビッグバン☆ソング"]],
      ["curated-160830", ["絶対宇宙スタンダード"]],
      ["curated-161632", ["普変"]],
      ["curated-158870", ["めざせポケモンマスター -with my friends-", "ひゃくごじゅういち", "タケシのパラダイス", "ラプラスにのって", "前向きロケット団！", "タイプ：ワイルド"]],
      ["curated-157883", ["ひろがるスカイ！プリキュア ～Hero Girls～", "ヒロガリズム", "Dear Shine Sky", "うれしくて", "All for one Forever", "ヒロガリズム ～プリキュア・クインテットVer.～"]]
    ]);
    for (const [animeId, titles] of reviewedMusic) {
      const anime = curatedAnimeDetails.find(({ id }) => id === animeId);
      expect(anime?.themes.map(({ titleJa }) => titleJa), animeId).toEqual(titles);
    ...
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-155997")?.themes).toHaveLength(0);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-199796")?.themes).toHaveLength(0);
  });
  ...
  it("keeps the reviewed 2023 fall boundary and multi-theme identities conservative", () => {
    const fall = curatedSeasonDetails.find((season) => season.id === "2023-fall");
    expect(fall?.anime).toHaveLength(100);
    const fallSeeds = curatedAnimeSeeds.filter((seed) => seed.seasonIds.includes("2023-fall"));
    expect(fallSeeds).toHaveLength(100);
    expect(new Set(fallSeeds.map(({ anilistId }) => anilistId)).size).toBe(100);
    expect(new Set(fallSeeds.map(({ slug }) => slug)).size).toBe(100);
    expect(fallSeeds.flatMap(({ themes }) => themes)).toHaveLength(200);

    for (const seed of fallSeeds) {
      expect(seed.startDate).toMatch(/^2023-(?:09|10|11|12)-\d{2}$/);
      expect(seed.startDate >= "2023-09-25" && seed.startDate <= "2023-12-29").toBe(true);
      const editorialWeekday = new Date(`${seed.startDate}T00:00:00Z`).getUTCDay() || 7;
      expect(seed.editorialWeekday, `${seed.anilistId} weekday`).toBe(editorialWeekday);
      expect(seed.titleJa.trim()).not.toBe("");
      expect((seed.titleZhHant ?? "").trim()).not.toBe("");
    ...
    const fallIds = new Set(fall?.anime.map((anime) => anime.id));
    for (const excludedAniListId of [169748, 170535, 170620, 177465, 170221]) {
      expect(fallIds.has(`curated-${excludedAniListId}`)).toBe(false);
    }
    expect(curatedAnimeDetails.filter((anime) => (
      fallIds.has(anime.id) && anime.themeAvailability === "not_announced"
    ...
    expect(curatedAnimeDetails.filter((anime) => fallIds.has(anime.id))
      .flatMap((anime) => anime.themes)
      .flatMap((theme) => theme.videos)).toHaveLength(2);

    const reviewedMusic = new Map([
      ["curated-163364", ["コレクション", "青春☆ワチャゴナドゥ", "祈り"]],
      ["curated-163204", ["ヒカリトカゼ", "Days of Birth"]],
      ["curated-127163", ["pas @ pas！"]],
      ["curated-167820", ["Black Crack", "salvia"]],
      ["curated-170206", ["bloom"]],
      ["curated-162147", ["君の居場所（Have a Good Time Here）"]]
    ]);
    for (const [animeId, titles] of reviewedMusic) {
      const anime = curatedAnimeDetails.find(({ id }) => id === animeId);
      expect(anime?.themes.map(({ titleJa }) => titleJa), animeId).toEqual(titles);
    ...
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-170206")
      ?.themes[0]?.videos[0]).toMatchObject({
        youtubeVideoId: "QjQym1J9Qtw",
        channelName: "Netflix Japan",
        officialStatus: "official"
      });
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-162147")
      ?.themes[0]?.videos[0]).toMatchObject({
        youtubeVideoId: "pQR4xBEM11Q",
        channelName: "Netflix Japan",
        officialStatus: "official"
      });

    const frieren = curatedAnimeDetails.find((anime) => anime.id === "curated-154587");
    expect(frieren?.titleZhHant).toBe("葬送的芙莉蓮");
    expect(frieren?.themes.map(({ type, sequence, titleJa, artistDisplayName }) => ({
      type,
      sequence,
      titleJa,
      artistDisplayName
    }))).toEqual([
      { type: "OP", sequence: 1, titleJa: "Yuusha", artistDisplayName: "YOASOBI" },
      { type: "OP", sequence: 2, titleJa: "Haru", artistDisplayName: "Yorushika" },
      { type: "ED", sequence: 1, titleJa: "Anytime Anywhere", artistDisplayName: "milet" },
      { type: "ED", sequence: 2, titleJa: "bliss", artistDisplayName: "milet" }
    ]);
    expect(curatedAnimeDetails.find((anime) => anime.id === "curated-161645")?.themes).toHaveLength(4);
    expect(curatedAnimeDetails.find((anime) => anime.id === "curated-126316")?.themes).toHaveLength(10);
    expect(curatedAnimeDetails.find((anime) => anime.id === "curated-162209")?.themes).toHaveLength(11);
    expect(curatedAnimeDetails.find((anime) => anime.id === "curated-165159")?.themes).toHaveLength(9);
  });

  it("keeps the reviewed 2023 summer boundary and manually recovered song identities conservative", () => {
    const summer = curatedSeasonDetails.find((season) => season.id === "2023-summer");
    expect(summer?.anime).toHaveLength(75);
    const summerSeeds = curatedAnimeSeeds.filter((seed) => seed.seasonIds.includes("2023-summer"));
    expect(summerSeeds).toHaveLength(75);
    expect(new Set(summerSeeds.map(({ anilistId }) => anilistId)).size).toBe(75);
    expect(new Set(summerSeeds.map(({ slug }) => slug)).size).toBe(75);
    expect(summerSeeds.flatMap(({ themes }) => themes)).toHaveLength(126);

    for (const seed of summerSeeds) {
      expect(seed.startDate).toMatch(/^2023-(?:06|07|08|09)-\d{2}$/);
      expect(seed.startDate >= "2023-06-23" && seed.startDate <= "2023-09-21").toBe(true);
      const editorialWeekday = new Date(`${seed.startDate}T00:00:00Z`).getUTCDay() || 7;
      expect(seed.editorialWeekday, `${seed.anilistId} weekday`).toBe(editorialWeekday);
      expect(seed.titleJa.trim()).not.toBe("");
      expect((seed.titleZhHant ?? "").trim()).not.toBe("");
    ...
    const summerIds = new Set(summer?.anime.map((anime) => anime.id));
    for (const excludedAniListId of [158908, 162291, 168962, 169319, 169442]) {
      expect(summerIds.has(`curated-${excludedAniListId}`)).toBe(false);
    }
    expect(curatedAnimeDetails.filter((anime) => (
      summerIds.has(anime.id) && anime.themeAvailability === "not_announced"
    ))).toHaveLength(20);
    expect(curatedAnimeDetails.filter((anime) => summerIds.has(anime.id))
      .flatMap((anime) => anime.themes)
      .flatMap((theme) => theme.videos)).toHaveLength(4);

    const expectedThemes = new Map([
      ["curated-154789", ["鳴らせ！むじなシンフォニー", "オ・ヒ・メ・サ・マ！"]],
      ["curated-146646", ["The Beast", "Sarracenia", "WILDER", "Salvia"]],
      ["curated-160831", ["夏暁", "FLY & DIVE"]],
      ["curated-159926", ["NEW DAWN", "La Muse perdue", "Disappear"]],
      ["curated-146472", ["Vivid Venus"]]
    ]);
    for (const [animeId, titles] of expectedThemes) {
      const anime = curatedAnimeDetails.find(({ id }) => id === animeId);
      expect(anime?.themes.map(({ titleJa }) => titleJa), animeId).toEqual(titles);
      for (const theme of anime?.themes ?? []) {
        expect(theme.sources.some(({ role }) => role === "first_party"), theme.id).toBe(true);
    ...
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-146065")?.themes
      .map(({ titleJa }) => titleJa)).toEqual(["spiral", "Musubime", "Hanasakimi"]);
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-145064")?.themes).toHaveLength(4);

    expect(curatedAnimeDetails.find(({ id }) => id === "curated-154789")
      ?.themes.find(({ type }) => type === "ED")?.videos[0]).toMatchObject({
        youtubeVideoId: "zUNn0CsuZg4",
        channelName: "Neko Hacker / ネコハッカー",
        officialStatus: "official"
      });
    expect(curatedAnimeDetails.find(({ id }) => id === "curated-165426")
      ?.themes[0]?.videos[0]).toMatchObject({
        youtubeVideoId: "mR1JJWtq0HY",
        channelName: "アニメ『おでかけ子ザメ』チャンネル",
        officialStatus: "official"
      });
  });

  it("keeps the reviewed 2024 winter boundary and complex song identities conservative", () => {
    const winter = curatedSeasonDetails.find((season) => season.id === "2024-winter");
    expect(winter?.anime).toHaveLength(75);
    const winterSeeds = curatedAnimeSeeds.filter((seed) => seed.seasonIds.includes("2024-winter"));
    expect(winterSeeds).toHaveLength(75);
    expect(new Set(winterSeeds.map(({ anilistId }) => anilistId)).size).toBe(75);
    expect(new Set(winterSeeds.map(({ slug }) => slug)).size).toBe(75);
    expect(winterSeeds.flatMap(({ themes }) => themes)).toHaveLength(130);

    for (const seed of winterSeeds) {
      expect(seed.startDate).toMatch(/^2024-0[1-3]-\d{2}$/);
      expect(seed.startDate >= "2024-01-01" && seed.startDate <= "2024-03-22").toBe(true);
    ...
    const winterIds = new Set(winter?.anime.map((anime) => anime.id));
    expect(winterIds.has("curated-199478")).toBe(false);
    expect(curatedAnimeDetails.filter((anime) => (
      winterIds.has(anime.id) && anime.themeAvailability === "not_announced"
    ))).toHaveLength(26);
    expect(curatedAnimeDetails.filter((anime) => winterIds.has(anime.id))
      .flatMap((anime) => anime.themes)
      .flatMap((theme) => theme.videos)).toHaveLength(125);

    expect(curatedAnimeDetails.find((anime) => anime.id === "curated-153518")?.themes).toHaveLength(4);
    expect(curatedAnimeDetails.find((anime) => anime.id === "curated-139518")?.themes).toHaveLength(6);
    expect(curatedAnimeDetails.find((anime) => anime.id === "curated-166093")?.themes).toHaveLength(14);
    expect(curatedAnimeDetails.find((anime) => anime.id === "curated-171030")?.themes).toHaveLength(6);
    expect(curatedAnimeDetails.find((anime) => anime.id === "curated-151639")?.themes).toHaveLength(5);

    const soloLeveling = curatedAnimeDetails.find((anime) => anime.id === "curated-151807");
    expect(soloLeveling?.themes.filter(({ type }) => type === "OP")).toEqual([
      expect.objectContaining({
        sequence: 1,
        titleJa: "LEveL",
        artistDisplayName: "SawanoHiroyuki[nZk]: TOMORROW X TOGETHER"
      })
    ]);
    expect(curatedAnimeDetails.find((anime) => anime.id === "curated-160389")?.themes[0])
      .toMatchObject({ titleJa: "Lover's Eye", artistDisplayName: "Sizuk" });
  });
  ...
  it("keeps the reviewed 2024 fall boundary and theme data conservative", () => {
    const fall = curatedSeasonDetails.find((season) => season.id === "2024-fall");
    expect(fall?.anime).toHaveLength(88);
    const fallSeeds = curatedAnimeSeeds.filter((seed) => seed.seasonIds.includes("2024-fall"));
    expect(fallSeeds).toHaveLength(88);
    expect(new Set(fallSeeds.map(({ anilistId }) => anilistId)).size).toBe(88);
    expect(new Set(fallSeeds.map(({ slug }) => slug)).size).toBe(88);

    for (const seed of fallSeeds) {
      expect(seed.startDate).toMatch(/^2024-(?:09|10|11|12)-\d{2}$/);
      expect(seed.startDate >= "2024-09-27" && seed.startDate <= "2024-12-21").toBe(true);
      const weekday = new Date(`${seed.startDate}T00:00:00Z`).getUTCDay() || 7;
      expect(seed.editorialWeekday, `${seed.anilistId} weekday`).toBe(weekday);
      expect(seed.titleJa.trim()).not.toBe("");
      expect((seed.titleZhHant ?? "").trim()).not.toBe("");
    ...
    for (const excludedAniListId of [138522, 176369, 182434, 183383, 189834, 199479, 202422, 207031]) {
      expect(fallIds.has(`curated-${excludedAniListId}`)).toBe(false);
    }

    const dandadan = curatedAnimeDetails.find((anime) => anime.slug === "dandadan");
    expect(dandadan?.themes.map(({ type, titleJa, artistDisplayName }) => ({
      type,
      titleJa,
      artistDisplayName
    }))).toEqual([
      { type: "OP", titleJa: "オトノケ", artistDisplayName: "Creepy Nuts" },
      { type: "ED", titleJa: "TAIDADA", artistDisplayName: "Zutto Mayonaka de Ii no ni." }
    ]);
    expect(dandadan?.themes.flatMap((theme) => theme.videos).map(({ youtubeVideoId }) => youtubeVideoId)).toEqual([
      "tRwHpyOq4P4",
      "s8SSPUQvUzI"
    ]);

    const mechaUde = curatedAnimeDetails.find((anime) => anime.slug === "mecha-ude-tv");
    expect(mechaUde?.themes.map(({ type, sequence, titleJa }) => ({ type, sequence, titleJa }))).toEqual([
      { type: "OP", sequence: 1, titleJa: "VORTEX" },
      { type: "ED", sequence: 1, titleJa: "karma" },
      { type: "ED", sequence: 2, titleJa: "arma" }
    ]);

    const uzumaki = curatedAnimeDetails.find((anime) => anime.slug === "uzumaki");
    expect(uzumaki?.themes[0]?.sources).toEqual(expect.arrayContaining([
      expect.objectContaining({ url: "https://www.viz.com/uzumaki", role: "first_party" }),
      expect.objectContaining({ url: "https://animethemes.moe/anime/uzumaki", role: "cross_check" })
    ]));
  });

  it("keeps the reviewed 2025 winter boundary conservative", () => {
    const winter = curatedSeasonDetails.find((season) => season.id === "2025-winter");
    expect(winter?.anime).toHaveLength(59);

    const behemoth = curatedAnimeDetails.find((anime) => anime.id === "curated-176158");
    expect(curatedAnimeSeeds.find((anime) => anime.id === behemoth?.id)?.startDate).toBe("2024-12-28");
    expect(winter?.anime.some((anime) => anime.id === behemoth?.id)).toBe(true);

    const winterIds = new Set(winter?.anime.map((anime) => anime.id));
    expect(curatedAnimeDetails
      .filter((anime) => winterIds.has(anime.id) && anime.themeAvailability === "not_announced")
      .map((anime) => anime.slug)
      .sort()
    ).toEqual([
      "haai-myakumyaku-desu",
      "kankin-kuiki-level-x-2nd-season",
      "maomao-no-hitorigoto-2nd-season",
      "lets-roll-cinnamoroll",
      "monpoke",
      "sora-iro-utility-birdie-wing-golf-girls-story-mini-anime"
    ].sort());
  });

  it("preserves the manually resolved 2025 winter song credits", () => {
    const company = curatedAnimeDetails.find((anime) => anime.slug === "kono-kaisha-ni-suki-na-hito-ga-imasu");
    expect(company?.themes.find((theme) => theme.type === "ED")).toMatchObject({
      titleJa: "ふたりじめ",
      artistDisplayName: "pachae",
      videos: [expect.objectContaining({ youtubeVideoId: "C6fZ9BET0Z0", type: "creditless_ed" })]
    });

    const infant = curatedAnimeDetails.find((anime) => anime.slug === "0-saiji-start-dash-monogatari-season-2");
    expect(infant?.themes[0]).toMatchObject({
      titleJa: "Beyond the Trajectory",
      artistDisplayName: "Chata"
    });
    expect(infant?.themes[0]?.sources).toEqual(expect.arrayContaining([
      expect.objectContaining({ url: "https://x.com/shusuisha/status/1875376613787128118", role: "first_party" }),
      expect.objectContaining({ url: "https://ascii.jp/elem/000/004/243/4243811/4/", role: "cross_check" })
    ]));

    const wataru = curatedAnimeDetails.find((anime) => anime.slug === "mashin-souzouden-wataru");
    expect(wataru?.themes.map(({ type, sequence, titleJa, artistDisplayName }) => ({
      type,
      sequence,
      titleJa,
      artistDisplayName
    }))).toEqual([
      { type: "OP", sequence: 1, titleJa: "POP UP!", artistDisplayName: "lol -エルオーエル-" },
      { type: "OP", sequence: 2, titleJa: "創", artistDisplayName: "SANTA" },
      { type: "ED", sequence: 1, titleJa: "ポケット", artistDisplayName: "FANTASTICS" }
    ]);
    expect(wataru?.themes.flatMap((theme) => theme.videos).map((video) => video.youtubeVideoId)).toEqual([
      "d7b7oVxFm-0",
      "woGuV1Wcspg",
      "zPeU84Fk73I"
    ]);

    expect(curatedAnimeDetails.find((anime) => anime.slug === "yami-shibai-14")?.themes[0]?.titleJa).toBe("飴と鞭");
    expect(curatedAnimeDetails.find((anime) => anime.slug === "unnamed-memory-act-2")?.themes[0]?.artistDisplayName).toBe("TRUE");
    expect(curatedAnimeDetails.find((anime) => anime.slug === "akuyaku-reijou-tensei-oji-san")?.themes
      .map((theme) => theme.titleJa)).toEqual(["Choose!!!", "マツケンサンバⅡ", "銀河鉄道999"]);
  });
  ...
  it("uses traceable HTTPS artwork and reviewed public sources", () => {
    const latestCycleIds = new Set(
      curatedSeasonDetails
        .filter((season) => (
          ...
          || season.id === "2022-spring"
          || season.id === "2022-winter"
        ))
    ...
    for (const anime of curatedAnimeDetails) {
      ...
      expect(anime.sources.some((source) => source.role === "first_party"), anime.id).toBe(true);
      expect(anime.sources.some((source) => source.role !== "first_party"), anime.id).toBe(true);
      ...
      for (const item of anime.themes) {
        const expectedVerifiedAt = ["catalog-cinderella-girls-climax-2019", "catalog-hangyakusei-million-arthur-2-2019", "catalog-king-of-prism-sss-2019", "catalog-aikatsu-friends-2-2019", "catalog-kiratto-prichan-2-2019"].includes(anime.id)
          ? "2026-09-08"
