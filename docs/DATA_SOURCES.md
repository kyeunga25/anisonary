...
每首已發布歌曲在 `PublicTheme.sources[]` 留下 URL、語言、`first_party`／`cross_check` 角色及核對日期。二十六季快照的 3,965 筆歌曲都必須同時具備兩種角色；這是公開 provenance gate，不是 confidence score。第三方索引將歌曲標為 OP／ED、但第一方頁面只證實插曲、劇中歌、Victory Dance、其他節目用途、舊季度歌曲、其他改編版本或一般「主題歌」時，不把它升格為官方 OP／ED；在既有導航契約中保留的一般主題曲會明示「主題歌（官方分類）」。完整欄位與媒體邊界見 [`DATA_PROVENANCE.md`](./DATA_PROVENANCE.md)。
