...
目前二十六季快照的 season references、1,808 個作品及 3,965 筆 OP／ED 都有結構化 provenance；3,965 筆歌曲全部至少保留一個 `first_party` 與一個 `cross_check` 來源。`tests/unit/curated-catalog.test.ts` 鎖定來源數量、角色、語言、HTTPS、核對日期、Mock URL 禁止條件，以及 legacy labels 與結構化 ledger 的一致性。
