...
2019 春季目前整理 17 套 TV 作品，夏季整理 38 套 TV 作品及 4 套網絡連載；兩季的其他作品、跨季延續與特殊歌曲仍待核對；`coverageNote` 會在季度頁及 API 公開說明本季範圍。

目前二十九季快照的 season references、1,934 個作品及 4,267 筆 OP／ED 都有結構化 provenance；4,267 筆歌曲全部至少保留一個 `first_party` 與一個 `cross_check` 來源。`tests/unit/curated-catalog.test.ts` 鎖定來源數量、角色、語言、HTTPS、核對日期、Mock URL 禁止條件，以及 legacy labels 與結構化 ledger 的一致性。
