...
### Phase 2 API contract gate production

| 項目 | 結果 |
|---|---|
| 日期 | 2026-07-25 |
| Git source | `codex/phase-2-curated-catalog@7de160e` |
| Delivery | `npm run cf:deploy` break-glass 手動 production deployment |
| Production version | `a4559d4a-7606-4caa-aef7-0e6507622bc3` |
| Rollback baseline | `8e98b7c3-18c7-4912-9899-a830d4125007` |
| Data mode | Repository-reviewed `CuratedProvider`；`PUBLIC_API_BASE_URL` 留空、`ANISONARY_REQUIRE_API_DATA=false` |
| Result | 首頁、兩季、動畫詳情、About、Sources、robots、sitemap 回應 `200`；未知 route 回應 `404` |

Production 首頁顯示「已核對精選目錄」且沒有 Mock Data 提示；custom domain 保留 `Referrer-Policy: strict-origin-when-cross-origin`、`X-Content-Type-Options: nosniff` 與 `X-Frame-Options: DENY`，`workers.dev` hostname 保留 `X-Robots-Tag: noindex`。這次手動 deployment 不改變 Workers Builds 仍以 `main` 作自動 production branch 的治理規則。
