...
|---|---|---|---|
| DEP-001 | Node／build／output 可重現 | Ready | Cloudflare 使用 Node 22、`npm run build`、`dist` |
| DEP-002 | GitHub CI 阻止不合格變更 | Ready | `main` branch protection 要求 `quality` check |
| DEP-003 | PR 可先驗證 | Ready | Git integration 為 PR／非 production branch 建 preview |
| DEP-004 | Production API 失敗不可發布殘缺網站 | Ready | Production 設 `ANISONARY_REQUIRE_API_DATA=true` |
| DEP-005 | Static SEO discovery | Ready | `/robots.txt` 及 `/sitemap-index.xml` 可讀 |
| DEP-006 | Preview 與 `pages.dev` 不被索引 | Ready | Response 有 `X-Robots-Tag: noindex` |
| DEP-007 | 基本 static security headers | Ready | Production response headers 通過 smoke test |
| DEP-008 | Custom domain 與 canonical 一致 | Config ready | Pages 綁定 `anisonary.k-y.cc`，`PUBLIC_SITE_URL` 相同 |
...
Cloudflare Pages 的 Production 環境：
...
在 API 尚未可用的 preview，可把 `PUBLIC_API_BASE_URL` 留空及使用：

```text
ANISONARY_REQUIRE_API_DATA=false
```

`ANISONARY_REQUIRE_API_DATA=true` 會令任何必要 API request failure 終止 build，避免只有首頁錯誤狀態、沒有季度頁及動畫頁的版本被視為成功部署。環境變數不得包含 API token；目前公開唯讀 API 契約不要求前端 secret。
